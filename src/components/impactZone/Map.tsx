...
const ShareButton = ({ placeName, formData, position }: { placeName: string; formData: any; position: number[] }) => {
  ...
  const displayLocation = placeName === t("impactZone:unknownLocation") || placeName === "Unknown Location" 
    ? `${position[0].toFixed(4)}, ${position[1].toFixed(4)}`
    : placeName;
  
  const shareTitle = t("impactZone:shareTitle", { location: displayLocation });
  ...
  const displayUrl = currentUrl.replace(/^https?:\/\//, '').split('?')[0];

  const iconButtonStyle = "p-2 sm:p-3 rounded-full bg-white/5 text-white/80 hover:text-white hover:shadow-[0_0_18px_rgba(107,127,215,0.8)] hover:bg-[#FFE87C]/20 transition-all duration-300";

  return (
    <div className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`${iconButtonStyle} bg-[rgba(59,130,246,0.9)] hover:bg-[rgba(59,130,246,1)]`}
        title={t("impactZone:shareButton")}
      >
        <svg className="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="currentColor" strokeWidth={2.2}>
          <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/>
        </svg>
      </button>
      
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 bg-blue-950 rounded-lg shadow-lg border border-white/10 p-3 sm:p-4 z-50 min-w-[180px] sm:min-w-[200px]">
          <div className="text-xs sm:text-sm text-white/80 mb-2 sm:mb-3">{t("impactZone:shareWith")}</div>
  ...
  const getTitle = () => {
    ...
    return (
      <div className="relative w-full">
        <span className={showShareButton ? "block pr-12 sm:pr-16" : "block"}>{titleText}</span>
        {showShareButton && (
