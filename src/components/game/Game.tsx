...
// ----------------------------------------------------------------------
// 1) Tipos locales para evitar dependencias externas
// ----------------------------------------------------------------------
interface AsteroidImage extends HTMLImageElement { ready: boolean }

// ----------------------------------------------------------------------
// 2) Estilos UI (encapsulados dentro del componente)
// ----------------------------------------------------------------------
const UISTYLES = `
...
// ----------------------------------------------------------------------
// 3) Lógica del juego (portada desde tu HTML y adaptada a React/Next)
// ----------------------------------------------------------------------

type EarthBlast = {
  x: number;      // posición X del impacto
  y: number;      // posición Y (encima del arco)
  t: number;      // tiempo transcurrido (frames)
  dur: number;    // duración total (frames)
  maxR: number;   // radio máximo de la onda
};

type SmokePlume = {
  t: number;               // frames
  dur: number;             // cuánto dura
  puffs: { x:number; y:number; r:number; vx:number; vy:number; a:number }[];
};


function startGame(canvas: HTMLCanvasElement, exitTo: (route?: string) => void) {
  // ===== Storage seguro =====
  const storage: Storage | { getItem: (k: string) => string | null; setItem: (k: string, v: string) => void; removeItem: (k: string) => void } = (() => {
    try {
      const t = "__test__"; localStorage.setItem(t, "1"); localStorage.removeItem(t); return localStorage;
    } catch {
  ...
  const resize = () => {
    ...
    canvas.width = CW = w * DPR;
    canvas.height = CH = h * DPR;
  ...
  // 🔥 Margen off-screen (culleo)
  const OFF_M = 220 * DPR;
  function shouldCull(r: any) {
    return r.entered && (r.x < -OFF_M || r.x > CW + OFF_M || r.y > CH + OFF_M || r.y < -OFF_M * 1.5);
  }
  ...
  function loadAst(src: string): AsteroidImage {
    ...
    img.src = src; img.ready = false; img.onload = () => { img.ready = true; };
    return img;
  }
  ...
  function pickAsteroidType() {
    const r = Math.random(); const a = AST_WEIGHTS.metal; const b = a + AST_WEIGHTS.ice;
  ...
  function showBanner(text: string, ms = 1800) {
    if (!banner) return;
    banner.textContent = text; (banner as HTMLElement).style.display = "block";
  ...
  function earthTopYAt(x: number) {
    const dx = x - EARTH.cx; if (Math.abs(dx) > EARTH.R) return Infinity; return EARTH.cy - Math.sqrt(EARTH.R * EARTH.R - dx * dx);
  }
 ...
 const state: any = {
  ...
  difficultyLevel: 2, 
  _thrust: false,
  earthBlasts: [] as EarthBlast[], // 👈 explosiones activas
  smokePlumes: [] as SmokePlume[], // 👈 humo persistente
};
  ...
  function updateHUD() {
    const total = 5; const hearts = "❤️".repeat(state.lives) + "🤍".repeat(Math.max(0, total - state.lives));
    if (hud) hud.textContent = `Puntos: ${state.score} · Mejor: ${best} · Vidas: ${hearts} · Nivel: ${state.level-1}`;
  }
  ...
  function makeRockShape(radius: number, sides: number) {
    const pts = []; for (let i = 0; i < sides; i++) { const a = (i / sides) * Math.PI * 2; const wobble = 0.72 + Math.random() * 0.5; pts.push({ x: Math.cos(a) * radius * wobble, y: Math.sin(a) * radius * wobble }); }
    return pts;
  }
  ...
  function update() {
    ...
    if (!state.over) {
      for (const r of state.rocks) {
        ...
        if (!r.entered) {
          const enteredVert = r.y > -10 * DPR; const enteredHorz = r.x > -10 * DPR && r.x < CW + 10 * DPR;
      ...
      for (let i = state.rocks.length - 1; i >= 0; i--) {
        const r = state.rocks[i]; const tooOld = r.entered && r.age > 20 * 60; // 20s aprox
        if (shouldCull(r) || tooOld) { state.rocks.splice(i, 1); state.score += 5; updateHUD(); }
      }
    }

    // Colisión bala-roca con deflexión
    for (let i = state.rocks.length - 1; i >= 0; i--) {
  ...
  if (r.y + r.r > yArc) {
    ...
    if (!state.over && state.rocks.length === 0) { state.level++; spawnWave(); showBanner(`Nivel ${state.level} ↑`, 1200); updateHUD(); }
...
for (let i = state.smokePlumes.length - 1; i >= 0; i--) {
  ...
  for (const p of plume.puffs) {
    p.x += p.vx;
    p.y += p.vy;
    p.r *= 1.004;        // crece suave
    p.vy -= 0.006 * DPR; // “flota” hacia arriba
    p.a *= 0.9985;       // se desvanece lento
  }
 ...
 function damageEarth(rock?: any, yArc?: number) {
  ...
  // 2) crear explosión si tenemos posición
  if (rock && Number.isFinite(yArc)) {
    const blast: EarthBlast = {
      x: rock.x,
      y: yArc!,                 // justo en el borde del arco
      t: 0,
      dur: 46,                  // ~0.75s a 60fps
      maxR: Math.max(120, rock.r * 6), // onda más grande si la roca era grande
    };
    state.earthBlasts.push(blast);

    // --- Humo persistente (plume) ---
if (rock && Number.isFinite(yArc)) {
  const puffCount = 16 + ((rock.r / (20 * DPR)) * 10 | 0);
  const puffs: SmokePlume["puffs"] = [];
  for (let i = 0; i < puffCount; i++) {
    const angle = Math.PI * (0.75 + Math.random() * 0.5); // sale hacia arriba
    ...
    puffs.push({
      ...
      a: 0.22 + Math.random() * 0.18, // alpha inicial
    });
  }
  state.smokePlumes.push({
    t: 0,
    dur: 60 * 25, // ~25s
    puffs,
  });
    ...
    // 2.1) chispas y humo alrededor del impacto
    for (let k = 0; k < 40; k++) {
      const a = Math.random() * Math.PI + Math.PI; // hemisferio “hacia arriba”
      ...
      const p = { 
        ...
        vx: Math.cos(a) * s, 
        vy: Math.sin(a) * s * 0.6, 
        life: 24 + (Math.random() * 18)|0, 
        color: k % 3 ? 'rgba(255,196,64,1)' : '#cbd5e1'
      };
      state.particles.push(p);
    }
  }

  // 3) mensajes / game over
  if (state.lives <= 0) {
    state.over = true;
    if (state.score > best) {
      best = state.score; 
      (storage as any).setItem?.('dartkids_best', String(best));
      showBanner(`🎉 ¡Nuevo récord! ${best} puntos`);
    } else {
      showBanner(`Juego terminado · Puntos: ${state.score} · Nivel: ${state.level}`);
    }
  } else {
    showBanner('¡Impacto! 🌋 Un asteroide tocó la Tierra', 1400);
  }
}
  ...
  function draw() {
    ...
    drawEarth();
    drawSmokePlumes(); // 👈 humo arriba del planeta
     drawShip(); drawBullets(); drawRocks(); drawParticles();
  }

  function drawEarth() {
    const { cx, cy, R } = EARTH; ctx.save(); ctx.beginPath(); ctx.arc(cx, cy, R, 0, Math.PI * 2); ctx.clip();
    if (earthReady) { ctx.translate(cx, cy); ctx.rotate(state.earthAngle); const S = R * 2.2; ctx.drawImage(earthTex, -S / 2, -S / 2, S, S); ctx.setTransform(1, 0, 0, 1, 0, 0); }
    else { const g = ctx.createRadialGradient(cx - R / 4, cy - R / 4, R * 0.5, cx, cy, R); g.addColorStop(0, "#1d4ed8"); g.addColorStop(1, "#0c4a6e"); ctx.fillStyle = g; ctx.fillRect(cx - R, cy - R, R * 2, R * 2); }
    ctx.restore(); ctx.beginPath(); ctx.arc(cx, cy, R * 1.02, 0, Math.PI * 2); ctx.fillStyle = "rgba(96,165,250,0.04)"; ctx.fill();
  }
  function drawSmokePlumes() {
  if (!state.smokePlumes.length) return;

  for (const plume of state.smokePlumes) {
    const life = 1 - Math.min(1, plume.t / plume.dur); // 1..0

    for (const p of plume.puffs) {
      // Clip al disco de la Tierra para que parezca que sale del borde
      ctx.save();
      ctx.beginPath();
      ctx.arc(EARTH.cx, EARTH.cy, EARTH.R, 0, Math.PI * 2);
      ctx.clip();

      ctx.globalAlpha = Math.max(0, p.a * (0.6 + 0.4 * life));
      const grad = ctx.createRadialGradient(p.x, p.y, p.r * 0.2, p.x, p.y, p.r);
      grad.addColorStop(0, 'rgba(148,163,184,0.45)'); // gris suave
      grad.addColorStop(1, 'rgba(15,23,42,0)');       // se esfuma
      ctx.fillStyle = grad;
      ctx.beginPath();
  ...
  function drawRocks() {
    for (const r of state.rocks) {
      if (r.trail && r.trail.length > 1) {
        ctx.save(); ctx.beginPath(); for (let t = 0; t < r.trail.length; t++) { const p = r.trail[t]; if (t === 0) ctx.moveTo(p.x, p.y); else ctx.lineTo(p.x, p.y); }
  ...
  // ----- Sprites nave -----
function loadShip(src: string) {
  const img = new Image() as HTMLImageElement & { ready?: boolean };
  img.src = src;
  img.ready = false;
...
const shipSprites = {
  idle: loadShip("/ship_idle.png"),
  thrust: loadShip("/ship_thrust.png"),
};
...
function drawShip() {
  ...
  if (SHIP_STYLE === "sprite") {
    const tex = state._thrust && shipSprites.thrust.ready 
      ? shipSprites.thrust 
      : shipSprites.idle;
    ...
    if (tex && tex.ready) {
      ...
      const W = s.w * 1.9, H = s.h * 1.9;
      ctx.drawImage(tex, -W / 2, -H * 0.6, W, H);
      ctx.restore();
      return; // 👈 aquí se usa SHIP_STYLE
    }
  }

  // fallback vectorial (triángulo con llama)
  ctx.save();
  ctx.translate(s.x, s.y);
  ctx.beginPath();
  ctx.moveTo(0, -s.h * 0.6);
  ctx.lineTo(s.w * 0.35, s.h * 0.4);
  ctx.lineTo(-s.w * 0.35, s.h * 0.4);
  ctx.closePath();
  ...
  for (const b of state.bullets) {
    ...
    grad.addColorStop(0, '#ffea00');   // inicio amarillo
    grad.addColorStop(1, '#ff0077');   // fin rosa neón
    ctx.strokeStyle = grad;
    ctx.lineWidth = 4 * DPR;
    ctx.beginPath();
    ctx.moveTo(b.x, b.y);
    ctx.lineTo(b.x, b.y - 20 * DPR);
    ctx.stroke();
  }
}
  ...
  function drawParticles() {
    for (let i = state.particles.length - 1; i >= 0; i--) {
      const p = state.particles[i]; p.x += p.vx; p.y += p.vy; p.life--; if (p.life <= 0) { state.particles.splice(i, 1); continue; }
      (ctx as any).globalAlpha = Math.max(0, p.life / 26); (ctx as any).fillStyle = p.color; ctx.fillRect(p.x, p.y, 3 * DPR, 3 * DPR); (ctx as any).globalAlpha = 1;
    }
  }
  ...
  function loop(now: number) {
    const dt = Math.min(0.033, (now - last) / 1000); last = now; // dt reservado si lo necesitas
  ...
  // Inicio
  function reset() {
    state.ship.x = (globalThis as any).innerWidth * DPR / 2 || CW / 2; state.ship.y = CH - 140 * DPR;
    ...
    spawnWave(); updateHUD(); showBanner('Mueve la nave · Dispara a los asteroides · ¡Protege la Tierra! 🌍');
  }
...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const cleanup = startGame(canvasRef.current, (route = "/") => router.push(route));
    return cleanup;
  }, [router]);
  ...
  return (
    <section className="gameRoot">
      <div className="gameHeader">
        <div><b>{t('appGame.title', { defaultValue: 'DART MISION' })}</b> 🚀 {t('appGame.tagline', { defaultValue: '¡Protege la Tierra!' })}</div>
        <div id="hud" style={{ fontSize: "18px" }}>
          {t('hudGame.points', { defaultValue: 'Puntos' })}: 0 · {t('hudGame.best', { defaultValue: 'Mejor' })}: 0 · {t('hudGame.lives', { defaultValue: 'Vidas' })}: ❤️❤️❤️❤️❤️ · {t('hudGame.level', { defaultValue: 'Nivel' })}: 1
        </div>
      </div>

      <div className="gamePanel">
        <button className="btn" id="btnRestart">{t('ui.restart', { defaultValue: 'Reiniciar' })}</button>
        <button className="btn" id="btnHelp">{t('ui.howToPlay', { defaultValue: '¿Cómo jugar?' })}</button>
        <button className="btn btn-exit" id="btnExit" title="Salir (ESC)">{t('ui.exit', { defaultValue: 'Salir' })}</button>
      </div>

      <div className="gameTouch" id="touchCtrls" style={{ display: 'none' }}>
        <div className="pad" data-dir="-1">⟵</div>
        <div className="pad shoot" id="shootBtn">{t('ui.shoot', { defaultValue: 'DISPARAR' })}</div>
        <div className="pad" data-dir="1">⟶</div>
      </div>
      ...
      <div id="intro" className="intro-backdrop hidden">
        <div className="intro-card">
          <h1>{t('introGame.title', { defaultValue: '¿Qué fue la misión DART?' })}</h1>
          <div className="intro-grid">
            <section className="intro-section">
              <h2>{t('introGame.summaryTitle', { defaultValue: 'Resumen' })}</h2>
              <p style={{ margin: '6px 0 0', fontSize: '14px', lineHeight: 1.55 }}>
                {t('introGame.summary', { defaultValue: 'La misión DART utilizó la técnica del impacto cinético para desviar un asteroide.' })}
              </p>
              <ul className="intro-list">
                <li><span className="kbd">←</span>/<span className="kbd">→</span> {t('introGame.move', { defaultValue: 'mover' })}</li>
                <li><span className="kbd">Espacio</span> {t('introGame.shoot', { defaultValue: 'disparar' })}</li>
                <li><span className="kbd">R</span> {t('introGame.restart', { defaultValue: 'reiniciar' })} · <span className="kbd">H</span> ayuda · <span className="kbd">P</span> {t('introGame.pause', { defaultValue: 'pausar' })} · <span className="kbd">ESC</span> {t('introGame.exit', { defaultValue: 'salir' })}</li>
              </ul>
            </section>
            <section className="intro-section">
              <h2>{t('introGame.tipsTitle', { defaultValue: 'Consejos para jugar' })}</h2>
              <ul className="intro-list">
                <li><b>{t('tips.anticipate.title', { defaultValue: 'Anticípate:' })}</b> {t('tips.anticipate.body', { defaultValue: 'dispara cuando el asteroide está alto.' })}</li>
                <li><b>{t('tips.position.title', { defaultValue: 'Colócate debajo:' })}</b> {t('tips.position.body', { defaultValue: 'muévete lo justo bajo su trayectoria.' })}</li>
                <li><b>{t('tips.rate.title', { defaultValue: 'Ritmo de disparo:' })}</b> {t('tips.rate.body', { defaultValue: 'hay enfriamiento; apunta bien.' })}</li>
                <li><b>{t('tips.priority.title', { defaultValue: 'Prioriza el peligro:' })}</b> {t('tips.priority.body', { defaultValue: 'si va centrado, va primero.' })}</li>
                <li><b>{t('tips.relax.title', { defaultValue: 'Relájate y disfruta:' })}</b> {t('tips.relax.body', { defaultValue: 'si fallás, reiniciás y a la otra sale mejor.' })}</li>
              </ul>
            </section>
          </div>
          <div className="intro-actions">
            <button className="btn-cta" id="btnPlay">{t('ui.play', { defaultValue: '¡Jugar!' })}</button>
            <button className="btn" id="btnExitIntro">{t('ui.exit', { defaultValue: 'Salir' })}</button>
          </div>
          <div className="intro-footer">
            <label><input type="checkbox" id="chkSkipIntro" /> {t('introGame.skip', { defaultValue: 'No mostrar esta intro la próxima vez' })}</label>
            <small>{t('introGame.tip', { defaultValue: 'Tip: tienes 5 vidas.' })}</small>
          </div>
        </div>
      </div>
